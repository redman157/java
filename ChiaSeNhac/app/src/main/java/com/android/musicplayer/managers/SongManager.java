package com.android.musicplayer.managers;
...
import com.android.musicplayer.database.AllPlaylist;
import com.android.musicplayer.database.CategorySongs;
import com.android.musicplayer.database.SongOfPlayList;
import com.android.musicplayer.database.Statistic;
...
public class SongManager {
    ...
    public void initDatabase(){
        ...
        mStatistic = new Statistic(context);
        mSharedPrefsUtils = new SharedPrefsUtils(context);

        totalSongs = mSharedPrefsUtils.getInteger(Constants.PREFERENCES.TOTAL_SONGS, -1);
...
/*    public ArrayList<HashMap<String, String>> artists() {
    ...
    */
    ...
    public void addToQueue(SongModel song) {
        queue().add(song);
        Utils.ToastLong(context, "Added to current queue!");
    }
    ...
    public void addToQueue(ArrayList<SongModel> arrayList) {
        ...
        if (arrayList1.size() > 0) {
            ...
            Utils.ToastLong(context, "Nothing to add");
        }
    }

    public void playNext(SongModel song) {
        queue().add(getCurrentMusic() + 1, song);
        Utils.ToastLong(context, "Playing next: " + song.getSongName());
    }
    ...
    public  void shufflePlay(int id, ArrayList<SongModel> array) {
        ...
        if (arrayList.size() > 0) {
            ...
            play(0, arrayList);
            Utils.ToastLong(context,"Shuffling");
        }
    }
    ...
    public void shufflePlay(ArrayList<SongModel> songs) {
        ...
        if (data.size() > 0) {
            ...
            Utils.ToastLong(context,"Nothing to shuffle");
        }
    }
    ...
    public void addMusic(SongModel song){
        if (mSongOfPlayList.searchSong(song)){
            ...
            Toast.makeText(context, "Đã Thêm Bài hát vào PlayList: "+song.getSongName(),
                    Toast.LENGTH_SHORT).show();
        }
    }

    public void addPlayList(String playList){
        if (mAllPlaylist.searchPlayList(playList)){
            Utils.ToastShort(context, "PlayList Đã Add Rồi");
        }else {
            mAllPlaylist.addPlayList(playList);
            Utils.ToastShort(context, "Add PlayList: "+playList);
    ...
    private void crawlData() {
        ...
        if (musicCursor != null) {
            ...
            mSharedPrefsUtils.setInteger(Constants.PREFERENCES.TOTAL_SONGS, mainList.size());
            musicCursor.close();
        }
