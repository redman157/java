...
public class EqualizerActivity extends AppCompatActivity implements OnClickListener,
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        initView();
        initData();
        mCurrentEqProfile = mSharedPrefsManager.getInteger(Constants.PREFERENCES.currentEqProfile
                , 0);
        assignView();
    ...
    private void setupEqualizerFxAndUI(boolean isCheck){
        try {
            Equalizer equalizer = new Equalizer(0,
                    mSharedPrefsManager.getInteger(Constants.PREFERENCES.audio_session_id, 0));
...
//                startActivity(new Intent(this, HomeActivity.class));
    ...
    public void onClickItem(String item, int pos) {
        ...
        if (mCurrentEqProfile != pos) {
            mSharedPrefsManager.setInteger(Constants.PREFERENCES.currentEqProfile, pos);
    ...
    public void onCheckedChanged(CompoundButton buttonView, boolean isChecked) {
        Log.d("CCC",
                "Equalizer --- onCheckedChanged: "+mSharedPrefsManager.getInteger(Constants.PREFERENCES.audio_session_id, 0) );
        mSharedPrefsManager.setBoolean(Constants.PREFERENCES.turnEqualizer, isChecked);
        Equalizer equalizer = new Equalizer(0,
                mSharedPrefsManager.getInteger(Constants.PREFERENCES.audio_session_id, 0));
        ...
        if (isChecked){
            for (int index = 0; index < Constants.VALUE.MAX_SLIDERS; index++){
                equalizer.setBandLevel((short) index,
                        (short) mSharedPrefsManager.getInteger("profile"+mCurrentEqProfile+"Band"+ index,0));
            }
            bassBoost.setStrength((short) mSharedPrefsManager.
                    getInteger("bassLevel" + mCurrentEqProfile, 0));
            virtualizer.setStrength((short) mSharedPrefsManager.
    ...
    public void onProgressChanged(SeekBar seekBar, int progress, boolean fromUser) {
        Equalizer eq = new Equalizer(0, mSharedPrefsManager.getInteger(Constants.PREFERENCES.audio_session_id, 0));
        BassBoost bassBoost = new BassBoost(0,mSharedPrefsManager.getInteger(Constants.PREFERENCES.audio_session_id, 0));
        Virtualizer virtualizer = new Virtualizer(0, mSharedPrefsManager.getInteger(Constants.PREFERENCES.audio_session_id, 0));
        if (seekBar == mBassBoost){
            ...
            mSharedPrefsManager.setInteger(Constants.PREFERENCES.bassLevel + mCurrentEqProfile, progress);
            ...
            mSharedPrefsManager.setInteger(Constants.PREFERENCES.vzLevel + mCurrentEqProfile, progress);
        }else {
            ...
            for (int i = 0; i < num_sliders; i++) {
                if (sliders[i] == seekBar) {
                    ...
                    mSharedPrefsManager.setInteger("profile" + mCurrentEqProfile + "Band" + i, new_progress);
                    break;
                }
            }
        }
    ...
    public void updateSliders() {
        try {
            for (int i = 0; i < num_sliders; i++) {
                int level =
                        (short) mSharedPrefsManager.getInteger("profile" + mCurrentEqProfile + "Band" + i,
                                0);
    ...
    public void updateBassBoost() {
        try {
            mBassBoost.setProgress((short)
                    mSharedPrefsManager.getInteger(Constants.PREFERENCES.bassLevel + mCurrentEqProfile,
                            0));
        } catch (Exception ignored) {
        }
    }
    ...
    public void updateVirtualizer() {
        try {
            mVirtualizer.setProgress((short)
                    mSharedPrefsManager.getInteger(Constants.PREFERENCES.vzLevel + mCurrentEqProfile, 0));
        } catch (Exception ignored) {
        }
    }
    ...
    public void updateUI() {
        ...
        updateBassBoost();
        updateVirtualizer();
        mSwEnabled.setChecked(mSharedPrefsManager.getBoolean(Constants.PREFERENCES.turnEqualizer, false));
    }
    ...
    public void setFlat() {
        try {
            ...
            for (int i = 0; i < num_sliders; i++) {
                ...
                mSharedPrefsManager.setInteger("profile" + mCurrentEqProfile + "Band" + i, 0);
            }
            mSharedPrefsManager.setInteger(Constants.PREFERENCES.bassLevel + mCurrentEqProfile, 0);
            mSharedPrefsManager.setInteger(Constants.PREFERENCES.vzLevel + mCurrentEqProfile, 0);
            updateUI();
            eq.release();
            bassBoost.release();
            virtualizer.release();
