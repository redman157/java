 ...
 */
class AspectRatio private constructor(val x: Int, val y: Int)
    : Comparable<AspectRatio>, Parcelable {

    fun matches(sizeCamera: SizeCamera): Boolean {
        val gcd = gcd(sizeCamera.width, sizeCamera.height)
        val x: Int = sizeCamera.width / gcd
        val y: Int = sizeCamera.height / gcd
    ...
    override operator fun compareTo(@NonNull other: AspectRatio): Int {
        if (equals(other)) {
            return 0
    ...
    companion object {
        private val sCache: SparseArrayCompat<SparseArrayCompat<AspectRatio>> =
            SparseArrayCompat(16)
